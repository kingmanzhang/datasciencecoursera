...
```{r}
attach(mtcars)
cormatrix <- outer(mtcars, mtcars, Vectorize(cor))
heatmap(cormatrix)
...
```{r}
pairs(mtcars[,-1], gap = 0, pch = ".", cex = 5,  lower.panel = NULL,
...
remove disp and hp. The VIF now looks much better.
```{r}
fit <- lm(mpg ~ ., data = mtcars[,-c(2,3)])
sqrt(vif(fit))
```
