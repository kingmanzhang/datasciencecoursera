...
require(knitr)
knitr::opts_chunk$set(echo = TRUE, root.dir = "~/git/datasciencecoursera")
```
...
model <- train2(c(EN.TWITTER.PATH, EN.BLOGS.PATH, EN.NEWS.PATH), percent = 0.002, batch_size = 5000, ngrams = 4)

require(rlist)
mySave <- function(model, file = "mymodel.rdata"){
  triekeys <- get_keys(model$trie.word)
  triewords <- get_values(model$trie.word)
  trieprob <- get_values(model$trie.prob)
  list.save(list(keys = triekeys, words = triewords, prob = trieprob, ngram = model$ngram, top.unigram = model$top.unigram), file)
}
mySave(model)
...
savedModel <- loadModel(file = "mymodel.1percent.4gram.rdata")
#h <- hash()
...
# S <- 1000000
# x <- vector(mode = "character", length = S)
# L <- c(letters, " ", " ")
# for (i in 1:S) {
#   x[i] = paste(sample(L, 10, replace = FALSE), collapse = "")
# }
# y <- wordCount(x)
...
predictScalar(model, "a")
predictScalar(model, "i")
predictScalar(model, "at the same time")
predictScalar(model, "")
...
Assess model performance. We randonly sample the corpus, tokenize them into bigrams, trigrams, and N+1-grams, then use them to test whether the model can predict the last token from the first few tokens.
```{r}
testGrams <- function(files, percent = 0.0001, ngrams = 4, batch_size = 1000){
  h <- list()
  for (i in 1:ngrams) {
    h[[i]] = hash()
  }
...
# change the hash to a dataframe. Hash h contains count for unigram
hash2df <- function(h){
  df <- data.frame(n = numeric(), query = character(), expected = character())
  for (i in 2:length(h)){
    allkeys <- keys(h[[i]])
    allvalues <- values(h[[i]])
    allkeys_split = str_split(allkeys, " ")
    allkeys_query = sapply(allkeys_split, function(x) str_c(x[1:i-1], collapse = " ")) 
    allkeys_value = sapply(allkeys_split, function(x) x[i])
    # maybe we should weight repeatitions
    dfsubset = data.frame(n = i - 1, query = rep(allkeys_query, allvalues), expected = rep(allkeys_value, allvalues))
    df <- rbind(df, dfsubset)
  }
  df
}

testset <- testGrams(c(EN.TWITTER.PATH, EN.BLOGS.PATH, EN.NEWS.PATH))
Rprof()
predicted = predictNext(model, as.character(testset[testset$n==3,2][1:1000]))
accuracy = sum(predicted == testset[testset$n == 3, 3][1:1000]) / length(predicted)
```
