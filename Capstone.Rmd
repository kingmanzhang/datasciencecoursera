...
trimEndWord <- function(x) {
  splitted <- strsplit(x, " ")
  sapply(splitted, function(x) {paste(x[1 : length(x) - 1])})
}
...
train <- function(percent = 0.01, n = 4) {
    ...
    for (i in 1:n) {
        ...
        trie_add(wordtrie, keys = colnames(dtm), values = dtm$v)
#        ngram[[i]] = dtm
    }
    # compute frequency
    # ngram_count <- list()
    # for (i in 1:n) {
    #   dataframe <- data.frame(key = colnames(ngram[[i]]), count = ngram[[i]]$v)
    #   ngram_count[[i]] <- dataframe
    # }
    # rm(ngram) ## clean this out of memory
    # for (i in n:2){
    #   ngram_count[[i]] <- ngram_count[[i]] %>% mutate(prop <- function(x) {})
    # }
    wordtrie
}

dtm <- train(percent = 0.0001)

trie_prob <- function(trie, n) {
  keys <- get_keys(trie)
  keys_current <- keys[sapply(strsplit(keys, " "), length) == n]
  for (i in n:1) {
    values_current <- unlist(prefix_match(trie, keys_current))
    keys_trim <- trimEndWord(keys_current)
    values_trim <- unlist(prefix_match(trie, keys_current))
    if (i >= 2) {
      trie_add(trie, keys_current, values_current/values_trim)
    } else {
      trie_add(trie, keys_current, values_current/sum(values_current))
    }
    keys_current <- keys_trim
  }
  trie
}
