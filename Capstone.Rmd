...
We now start to explore n-gram distributions. We will 
```{r echo=FALSE}
dtm1 <- lapply(processedCorpuses, function(x){DocumentTermMatrix(x, list(tokenize = oneGramTokenizer))})
...
mostFreqNGram <- function(x) {
    ...
    wordList <- colnames(dtmComb)
    freq <- colSums(as.matrix(dtmComb))
...
mostFreqNGram(dtm1) %>% head(n = 15)
```

Most frequent 2-grams:
```{r echo=FALSE}
mostFreqNGram(dtm2) %>% head(n = 15)
```

Most frequent 3-grams:
```{r echo=FALSE}
mostFreqNGram(dtm3) %>% head(n = 15)
```
