...
freqCount <- function(h, file, ngrams = 4, batch_size = 1000) {
  LINE_PROCESSED = 0
  fcon <- file(file, "r")
  batch <- vector(mode = "character", length = 1000L)
  batch_i <- 1
  ...
  while(TRUE) {
    ...
    if (batch_i > batch_size | length(line) == 0) {
      print(paste(c("Total lines processed:", LINE_PROCESSED), collapse = " "))
...
#freqCount(h, "/Users/zhangx/git/datasciencecoursera/final/en_subsets/twitter_subset.txt")
