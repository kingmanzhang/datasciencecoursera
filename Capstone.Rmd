...
predictScalar <- function(model, newData) {
  ...
  if (n >= N) {
    ...
    candidates <- data.frame(candidatesKey, candidatesValue) %>% arrange(-candidatesValue)
    if (nrow(candidates) > 0) {
      return (str_split(candidates[1,1], " ")[[1]][n + 1])
    } else {
      return (predictScalar(model, str_replace(newData, "^\\w+ ", "")))
    }
  }
}

predictFromNgram(model, "at the")
